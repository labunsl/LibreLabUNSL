...
/// 7   hx711
/// 8   hx711
/// 9   salida digital (PWM)
/// 10  salida digital (PWM)
...
flt lee_sensor(char id)
{
  ...
  switch(id)
  {
    ...
    case 7:   a=digitalRead(9); // 0 o 1
              error=0;
              break;
    
    case 8:   a=analogRead(A0); // 0 a 1023 (10 bits ADC)
              error=0;
              break;
...
// Detiene todas las interrupciones
    if(bufferEnt.indexOf("Stopint")==0) { detachInterrupt(digitalPinToInterrupt(2)); detachInterrupt(digitalPinToInterrupt(3)); }
