...
///     - agregada la posibilidad de dos sensores bmp280/bme280 sobre bus SPI comun (el primero con IDs. 1,2 y el segundo con IDs. 8,9)
///
/// Tareas a hacer:
///     - agregar la posibilidad de tener datos invalidos (ahora siempre tiene que haber un dato cuando muestrea)
///     - agregar comando que devuelva version de firmware
///
///
...
/// Para sensor presion/temperatura BMP280
// #include <SFE_BMP180.h>
// #include <Wire.h>
// SFE_BMP180 bmp180;  // pines predeterminados son A4 y A5


/// Para (doble) sensor presion/temperatura BMP280/BME280 
//#include <Wire.h>
//#include <BlueDot_BME280.h>
//BlueDot_BME280 bme1;          // Object for Sensor #1
//BlueDot_BME280 bme2;          // Object for Sensor #2
//int bme1_detected = 0;        // Flag if Sensor #1 is available
//int bme2_detected = 0;        // Flag if Sensor #2 is available

/// Para (doble) sensor presion/temperatura BMP280/BME280 (Adafruit)
#include <SPI.h>
...
int bmp1_detected = 0;        // Flag if Sensor #1 is available
int bmp2_detected = 0;        // Flag if Sensor #2 is available
...
/// A1  CSB2 bmp280 #1 y #2
/// A2  CSB1 bmp280 #1 y #2
/// A3  SDA  bmp280 #1 y #2
/// A4  SDO  bmp280 #1 y #2
/// A5  SCL  bmp280 #1 y #2
/// 
...
//  bmp180.begin();
  bmp1_detected = bmp1.begin();
  bmp2_detected = bmp2.begin();
...
flt lee_sensor(char id)
{
  ...
  switch(id)
  {
    ...
    case 1:   error=1;
              if(bmp1_detected)
              {   
                s=bmp1.readTemperature();   // obtener la temperatura BMP280 #1
                error=0;
              }
              break;
    ...
    case 2:   error=1;
              if(bmp1_detected)
              {   
                s=bmp1.readPressure();   // obtener la presion BMP280 #1
                error=0;
              }
              break;
    ...
    case 8:   error=1;
              if(bmp2_detected)
              {   
                s=bmp2.readTemperature();   // obtener la temperatura BMP280 #2
                error=0;
              }
              break;
    ...
    case 9:   error=1;
              if(bmp2_detected)
              {   
                s=bmp2.readPressure();   // obtener la presion BMP280 #2
                error=0;
              }
              break;
...
//
    ...
    if (bufferEnt.indexOf("Set") == 0)
    {
      ...
      if(!error)
      {
        ...
        if((param2<0) || (param2>9)) error+=16;     // controla rango de ID
